...
class UnwrappedFaceWeightedAverage(nn.Module):
    def __init__(self, output_num_channels=2, input_num_channels=3, inner_nc=512, arch='unet_64'):
        super(UnwrappedFaceWeightedAverage, self).__init__()

        self.pix2pixUnwrapped = Pix2PixModel(3, arch=arch)

        self.pix2pixSampler = NoSkipPix2PixModel(input_num_channels, output_num_channels, inner_nc=inner_nc, arch=arch)
    ...
    def forward(self, target_pose, *input_imgs):
        ...
        sampler_xy = sampler_xy.permute(0, 2, 3, 1) + stddev.permute(0, 2, 3, 1).contiguous().mul(
            Variable(torch.randn(xs.size()).cuda(), requires_grad=False)) + Variable(xs,
                                                                                     requires_grad=False)  # choose values according to the std dev
...
class UnwrappedFaceWeightedAveragePose(nn.Module):
    def __init__(self, output_num_channels=2, input_num_channels=3, inner_nc=512, input_pose=True):
        ...
        self.pix2pixSampler = NoSkipPix2PixModel_pose(input_num_channels, output_num_channels, input_pose=input_pose,
                                                      inner_nc=inner_nc)
